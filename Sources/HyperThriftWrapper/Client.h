 ...
 */
class Client : private ClientHelper, public ThriftGen::ClientServiceClient {
public:
  Client(const std::string &host, int port, int timeout_ms = 30000,
         bool open = true)
    : ClientHelper(host, port, timeout_ms), ClientServiceClient(protocol),
      m_do_close(false) {
...
/**
 * A client for the HQL Interpreter
 */
class HqlClient : private ClientHelper, public ThriftGen::HqlServiceClient {
public:
	HqlClient(const std::string &host, int port, int timeout_ms = 30000,
		   bool open = true)
	: ClientHelper(host, port, timeout_ms), ThriftGen::HqlServiceClient(protocol),
	m_do_close(false) {
